...
import { MINUTE, MONTH, SECOND } from "../date/time";
...
export default function AutoSave({
  ...
  editor: Editor | null;
  postHeader: Pick<Post, "title" | "en_title" | "tags">;
  ...
  const autoSave = useCallback(
    throttle(async ({ title, en_title, tags }: Pick<Post, "title" | "en_title" | "tags">) => {
      try {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="p-0" side="right" align="start">
        <div className="max-h-[400px] overflow-y-auto">
          {saves.length === 0 ? (
            <div className="p-4 text-center text-sm text-gray-500">임시저장된 내용이 없습니다.</div>
          ) : (
            saves.map((saveKey) => (
              <div
                key={saveKey[SESSION_ID]}
                className="flex cursor-pointer items-center justify-between border-b p-3 last:border-b-0 hover:bg-gray-100"
                onClick={() => loadSave(saveKey)}
              >
                <div className="flex flex-col">
                  <span className="max-w-[200px] truncate text-sm font-medium">{saveKey[TITLE] || "(제목 없음)"}</span>
